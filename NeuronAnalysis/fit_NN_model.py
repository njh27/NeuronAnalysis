...
FR_MAX = 500
""" POSITVE intervals move CS LTD function backward, before the CS and
NEGATIVE intervals move the CS LTD function forward, after the CS. """
CS_pair_interval = 0
...
def fit_learning_rates(NN_FIT, blocks, trial_sets, bin_width=10, bin_threshold=5):
    ...
    def learning_function(params, x, y):
        ...
        for trial in range(0, n_trials):
            ...
            W_pf[W_pf > W_max_pf[0], :] = W_max_pf[0]
            W_pf[W_pf < 0.0, :] = 0.0
            ...
            if UPDATE_MLI_WEIGHTS:
                ...
                W_mli[W_mli > W_max_mli[0], :] = W_max_mli[0]
                W_mli[W_mli < 0.0, :] = 0.0
    ...
    # For assymetric Gausian CS
        ...
        p0 =           np.array([10, 50, 10*np.amax(W_0_pf), 2*bin_width, 5*bin_width, 10.0,    5*bin_width,   5*bin_width,   40.0, 1.0, 10*np.amax(W_0_mli), 1.0, 50, 10])
