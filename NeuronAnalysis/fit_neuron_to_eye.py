...
class FitNeuronPositionPlanes(FitNeuronToEye):
    ...
    def fit_position_planes(self, knee_steps=[2.5, 0.25], bin_width=10, bin_threshold=1):
        ...
        fix_eye_data = np.nanmean(n_fit_position.get_fix_only_eye_traces(), axis=1)
        fix_fr_data = np.nanmean(n_fit_position.get_fix_only_fr_traces(), axis=1)
        all_eye_data = np.concatenate((all_eye_data, fix_eye_data), axis=0)
        all_fr_data = np.concatenate((all_fr_data, fix_fr_data), axis=0)
        purs_eye_data = np.nanmean(n_fit_position.get_purs_only_eye_traces()[:, 0:200, :], axis=1)
        purs_fr_data = np.nanmean(n_fit_position.get_purs_only_fr_traces()[:, 0:200], axis=1)
        all_eye_data = np.concatenate((all_eye_data, purs_eye_data), axis=0)
        all_fr_data = np.concatenate((all_fr_data, purs_fr_data), axis=0)
        purs_eye_data = np.nanmean(n_fit_position.get_purs_only_eye_traces()[:, 1100:1300, :], axis=1)
        purs_fr_data = np.nanmean(n_fit_position.get_purs_only_fr_traces()[:, 1100:1300], axis=1)
        all_eye_data = np.concatenate((all_eye_data, purs_eye_data), axis=0)
        all_fr_data = np.concatenate((all_fr_data, purs_fr_data), axis=0)
